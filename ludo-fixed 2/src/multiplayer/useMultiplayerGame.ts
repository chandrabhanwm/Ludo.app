...
export const useMultiplayerGame = (
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'games', gameId), (snap) => {
      ...
      gameStateRef.current = state; // sync update — always current before any re-render
      setGameState(state);
      ...
      // ── Disconnect detection (events + heartbeat) ──────────────
      if (state.status !== 'finished') {
        // Check heartbeats — if any player's heartbeat is >15s old, mark disconnected
        const now = Date.now();
        const stateData = snap.data() as any;
        if (!Array.isArray(state.players)) {
          // Diagnostic: log invalid state for investigation
          console.warn('[DISCONNECT-CHECK] state.players not array', {
            gameId,
            exists: snap.exists(),
            keys: Object.keys(stateData),
            playersType: typeof state.players,
            playersValue: state.players,
          });
        } else {
          for (const player of state.players) {
            if (player.uid === myUid || player.knockedOut || !player.connected) continue;
            const heartbeatKey = `heartbeat_${player.uid}`;
            const lastSeen = stateData[heartbeatKey];
            if (lastSeen && (now - lastSeen) > 15000) {
              updatePlayerConnection(gameId, player.uid, false, state.players).catch(() => {});
            }
          }
        }
      ...
      // ── Mark game started — fires when host writes status='ready' ──
      if (state.status === 'ready' && !gameStartedRef.current) {
  ...
  useEffect(() => {
    ...
    // Heartbeat — write lastSeen every 5s so opponent can detect stale presence
    const heartbeat = setInterval(async () => {
      const gs = gameStateRef.current;
      // Only write heartbeat if game is active AND has valid players
      // Prevents ghost doc creation after game ends/resets
      if (!gs || gs.status === 'finished' || !Array.isArray(gs.players)) return;
      try {
        const { setDoc, doc } = await import('firebase/firestore');
        const { db } = await import('../firebase/config');
        await setDoc(doc(db, 'games', gameId), {
          [`heartbeat_${myUid}`]: Date.now(),
          updatedAt: Date.now(),
        }, { merge: true });
      } catch {}
    }, 5000);
  ...
  // isMyTurn = it's my color's turn (true whether or not dice has been rolled)
  // hasRolled check is done at call sites, not here
  const isMyTurn = gameState?.activePlayerColor === myColor &&
    (gameState?.status === 'playing' || gameState?.status === 'countdown' || gameState?.status === 'ready');
  ...
  return {
    ...
    getLatestDiceValue: () => gameStateRef.current?.diceValue ?? 0,
  };
};
