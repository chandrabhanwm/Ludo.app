...
export const useMultiplayerGame = (
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'games', gameId), (snap) => {
      console.log('[LOG-6] onSnapshot fired', { exists: snap.exists() });
      if (!snap.exists()) return;
      const state = snap.data() as MultiplayerGameState;
      console.log('[LOG-7] before setGameState', { status: state.status, hasRolled: state.hasRolled, diceValue: state.diceValue, activeColor: state.activePlayerColor });
      setGameState(state);
  ...
  const rollDice = useCallback(async () => {
    ...
    console.log('[LOG-4] before writeDiceRoll', { gameId, rolled, newConsecutiveSixes });
    await writeDiceRoll(gameId, rolled, newConsecutiveSixes);
    console.log('[LOG-5] after writeDiceRoll succeeded');
    return rolled;
  }, [gameId]);
