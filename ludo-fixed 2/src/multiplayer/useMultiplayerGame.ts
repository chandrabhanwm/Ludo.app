...
export const useMultiplayerGame = (
  ...
  const gameStartedRef = useRef<boolean>(false); // prevent multiple markGameStarted calls
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'games', gameId), (snap) => {
      ...
      // Game countdown finished — mark as started (only once, by first player)
  ...
  // Set up presence detection — detects disconnect via network + visibility + unload
  useEffect(() => {
    if (!gameId || !isActive) return;
    ...
    // Page visibility (app backgrounded on mobile)
    ...
    // Mark as connected when hook activates
    markConnected(true);
  ...
  }, [gameId, myUid, isActive]);

  // Is it my turn? Allow during 'playing' OR just-transitioned from countdown
