          ...
          {status === 'menu' && onlineScreen === 'room' && activeTableId && (
            <motion.div key="room" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}}
              ...
              <TableRoom
                ...
                onGameStart={async (gameId, color, mode, tableNum) => {
                    ...
                    try {
                      ...
                      if (!snap.exists()) {
                        if (retriesLeft > 0) {
                          ...
                          console.error('Game doc never created after 5 retries — aborting board navigation');
                          setOnlineScreen('lobby'); // send back to lobby
                        }
                        return;
                      }
                      const gameDoc = snap.data();
                      setInitialGameState(gameDoc); // seed hook before listener fires
                      ...
                      setConsecutiveSixes(0);
                      setGameLogs([]);
                      // Set active player from Firestore game doc (not sync effect)
                      const activeColor = gameDoc.activePlayerColor || gameDoc.players?.[0]?.color;
                      const activeIdx = gamePlayers.findIndex(p => p.color === activeColor);
                      setCurrentPlayerIndex(activeIdx >= 0 ? activeIdx : gamePlayers.findIndex(p => p.type !== 'none'));
                      matchStartTime.current = Date.now();
