...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (gameMode === 'online') {
      if (hasRolled && multiplayer?.isMyTurn && !autoPassFiredRef.current) {
        const validMoves = getValidTokenMoves(activePlayer.color, diceValue);
        if (validMoves.length === 0) {
          // No valid moves — auto pass turn after delay (fires exactly once per turn)
      ...
      return; // don't run AI logic for online
    }
