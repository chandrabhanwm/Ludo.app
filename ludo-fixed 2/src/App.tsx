          ...
          {status === 'menu' && onlineScreen === 'room' && activeTableId && (
            <motion.div key="room" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}}
              ...
              <TableRoom
                ...
                onGameStart={async (gameId, color, mode, tNum, preloadedGameData?: any) => {
                  // Set multiplayer identifiers first so hook activates
                  setMultiplayerGameId(gameId);
                  setMyMultiplayerColor(color as PlayerColor);
                  setActiveTableMode(mode);
                  setActiveTableNum(tNum);

                  // Try to load game doc and init board
                  // preloadedGameData = host already has this, no Firestore read needed
                  const tryInit = async (retries = 8): Promise<void> => {
                    try {
                      let gd = preloadedGameData;
                      ...
                      if (!gd) {
                        ...
                        // Try cache first (fast), then server
                        let snap: any = null;
                        try {
                          snap = await getDocFromCache(gameRef);
                        } catch {
                          // Not in cache — read from server
                        }
                      ...
                      setInitialGameState(gd);

                      // Build players from game doc
                      ...
                      const gamePlayers: Player[] = allColors.map(c => {
                        const mp = gd.players?.find((p: any) => p.color === c);
                      ...
                      // Init all game state
                      const gameTokens = (gd.tokens || []).filter((t: any) => t.position !== 'knocked');
                      const activeColor = gd.activePlayerColor || gd.players?.[0]?.color;
                      const activeIdx = gamePlayers.findIndex(p => p.color === activeColor && p.type !== 'none');

                      setPlayers(gamePlayers);
                      setTokens(gameTokens.length > 0 ? gameTokens : allColors.flatMap(c =>
                        gamePlayers.find(p => p.color === c && p.type !== 'none')
                          ? [0,1,2,3].map(i => ({ id: `${c}-${i}`, playerColor: c, idInColor: i, position: 'yard' as const }))
                          : []
                      ));
                      setGameMode('online');
                      setMatchWinners(gd.matchWinners || []);
                      setDiceValue(gd.diceValue || 1);
                      setDiceState(gd.hasRolled ? 'rolled' : 'idle');
                      setHasRolled(gd.hasRolled || false);
                      setConsecutiveSixes(gd.consecutiveSixes || 0);
                      setIsAnimating(false);
                      setGameLogs([]);
