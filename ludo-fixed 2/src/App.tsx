...
export default function App() {
  ...
  }, [multiplayer?.gameState, gameMode, myMultiplayerColor]);
  ...
  const finalizeTokenMove = (tokenId: string, rolledValue?: number) => {
    ...
    if (gameMode === 'online' && multiplayer && multiplayerGameId) {
      ...
      multiplayer.makeMove(
        ...
        isExtraTurn, // hasRolled = true if same player rolls again
  ...
  const handleRollDice = async () => {
    // Online mode — animate locally then write to Firestore
    if (gameMode === 'online' && multiplayer) {
      if (!multiplayer.isMyTurn || hasRolled || diceState === 'rolling') return;
      // Show rolling animation immediately (better UX)
      setDiceState('rolling');
      await multiplayer.rollDice();
      // diceState/value will be set by sync effect when Firestore updates
      return;
    }
            ...
            <motion.div key="room" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}}
              ...
              <TableRoom
                ...
                onGameStart={async (gameId, color, mode) => {
                  ...
                  setActiveTableMode(mode);
                  // Keep onlineScreen='room' so TableRoom stays visible during init
                  // Will be cleared when board is ready

                  // Wait for game doc — retry up to 5 times with 1s intervals
                  const tryInitGame = async (retriesLeft = 5): Promise<void> => {
                    try {
                      ...
                      if (!snap.exists()) {
                        if (retriesLeft > 0) {
                          setTimeout(() => tryInitGame(retriesLeft - 1), 1000);
                        }
                        return;
                      }
                      ...
                      const gamePlayers: Player[] = allColors.map(c => {
                        ...
                        // Use color emoji as avatar — NOT photoURL (URLs render as raw text in HUD)
                        const initial = (mp.displayName || '?').charAt(0).toUpperCase();
                        return {
                          id: c,
                          name: mp.displayName || 'Player',
                          color: c,
                          type: isMe ? 'human' : 'online' as PlayerType,
                          avatar: COLOR_EMOJIS[c] || initial,
                          isWinner: false,
                        };
                      });
                      ...
                      matchStartTime.current = Date.now();
                      setOnlineScreen(null); // clear room screen JUST before showing board
                      setStatus('playing');
                    } catch (e) {
                      if (retriesLeft > 0) setTimeout(() => tryInitGame(retriesLeft - 1), 1000);
                      else console.error('Failed to initialize multiplayer game:', e);
                    }
                  };
                  setTimeout(() => tryInitGame(), 1500);
                }}
