...
export default function App() {
  ...
  const autoPassFiredRef = useRef<boolean>(false);  // prevents double auto-pass per turn
  const passTurnFiredRef = useRef<boolean>(false);   // makes passTurn idempotent
  ...
  const handleTokenSelect = (tokenId: string) => {
    ...
    if (gameMode === 'online' && !multiplayer?.isMyTurn) {
      console.log('[TOKEN] blocked - not my turn', { isMyTurn: multiplayer?.isMyTurn, activeColor: multiplayer?.gameState?.activePlayerColor, myColor: myMultiplayerColor, hasRolled: multiplayer?.gameState?.hasRolled });
      return;
    }
  ...
  useEffect(() => {
    ...
    if (gameMode === 'online') {
      // Use activePlayerColor match — isMyTurn is false when hasRolled=true (by design)
      const isMyColor = multiplayer?.gameState?.activePlayerColor === myMultiplayerColor;
      if (hasRolled && isMyColor && !autoPassFiredRef.current) {
        const validMoves = getValidTokenMoves(activePlayer.color, diceValue);
        if (validMoves.length === 0) {
