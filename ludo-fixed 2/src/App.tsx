...
export default function App() {
  ...
  const passTurn = () => {
    if (status !== 'playing') return;
    if (passTurnFiredRef.current) return; // idempotent — already fired this turn
      ...
      if (p.type !== 'none' && finishedCount < 4) {
        ...
        if (gameMode === 'online' && multiplayer && multiplayerGameId) {
          ...
          multiplayer.makeMove(
            '', // no token moved — just passing turn
            tokensRef.current,
            nextColor,
            matchWinners,
            0, // reset consecutiveSixes on pass
            false, // hasRolled = false for next player
