...
export default function App() {
  ...
  useEffect(() => {
    console.log('[LOG-8] sync useEffect ran', { gameMode, gsStatus: multiplayer?.gameState?.status, gsHasRolled: multiplayer?.gameState?.hasRolled, gsDiceValue: multiplayer?.gameState?.diceValue });
    ...
    // DIAGNOSTIC — print exact runtime values when Green becomes active
    if (gs.activePlayerColor === myMultiplayerColor) {
      console.log('[DIAG] Green is now active — dice disabled operands:', {
        activePlayerColor: multiplayer?.gameState?.activePlayerColor,
        myColor: myMultiplayerColor,
        isMyTurn: multiplayer?.isMyTurn,
        notIsMyTurn: !multiplayer?.isMyTurn,
        hasRolled,
        diceState,
        isRolling: diceState === 'rolling',
        isRolled: diceState === 'rolled',
        isAnimating,
        disabled:
    ...
    // Sync game status — when Firestore moves to 'playing', update local status
    if (gs.status === 'playing' && status !== 'playing') setStatus('playing');
  ...
  const passTurn = () => {
    console.log('[LOG-9] passTurn called', { status, passTurnFired: passTurnFiredRef.current });
    if (status !== 'playing') return;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (p.type !== 'none' && finishedCount < 4) {
        ...
        if (gameMode === 'online' && multiplayer && multiplayerGameId) {
          ...
          console.log('[LOG-10] passTurn makeMove called', { nextColor });
          multiplayer.makeMove(
            ...
            0,
            false,
          ).then(() => console.log('[LOG-11] passTurn makeMove succeeded'))
           .catch(e => console.error('[LOG-ERR] passTurn makeMove failed:', e));
        }
        return;
      }
  ...
  const handleRollDice = async () => {
    console.log('[LOG-1] handleRollDice entered');
    ...
    console.log('[DICE-DEBUG]', {
      ...
      activeTableId,
      myMultiplayerColor,
      firebaseUid: firebaseUserRef.current?.uid || null,
      gameStateStatus: multiplayer?.gameState?.status || null,
      gameStateActiveColor: multiplayer?.gameState?.activePlayerColor || null,
    });

    // Online mode — just record the roll to Firestore
    ...
    if (gameMode === 'online' && multiplayer) {
      ...
      try {
        console.log('[LOG-2] before multiplayer.rollDice()');
        await multiplayer.rollDice();
        console.log('[LOG-3] after multiplayer.rollDice()');
      } catch (e: any) {
  ...
  const handleTokenSelect = (tokenId: string) => {
    console.log('[TOKEN] handleTokenSelect', { tokenId, hasRolled, isAnimating, diceState, isMyTurn: multiplayer?.isMyTurn, gameMode });
    ...
    if (gameMode === 'online') {
      ...
      if (!isMyActiveColor) {
        console.log('[TOKEN] blocked - not my color', { activeColor: multiplayer?.gameState?.activePlayerColor, myColor: myMultiplayerColor });
        return;
      }
    }
