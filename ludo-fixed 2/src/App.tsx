...
export default function App() {
  ...
  const multiplayer = useMultiplayerGame(
    multiplayerGameId && activeTableId && myMultiplayerColor && firebaseUserRef.current?.uid ? {
      ...
      onGameEnd: (winners) => {
        ...
        // Award points and save stats for online game end
        // endMatch() handles all reward/stat logic — call it with online winners
        setTimeout(() => {
          // matchWinners state may not be set yet — pass winners directly via ref
          endMatchForOnline(winners);
        }, 100);
      },
    } : null
  );
  ...
  const autoPassFiredRef = useRef<boolean>(false);
  const passTurnFiredRef = useRef<boolean>(false);
  const prevActiveColorRef = useRef<string>(''); // tracks turn boundary
  ...
  useEffect(() => {
    ...
    if (!gs) return;
    if (gs.status !== 'playing' && gs.status !== 'countdown' && gs.status !== 'ready') return;
    ...
    if (gs.hasRolled && gs.diceValue > 0) {
      ...
      setHasRolled(false);
      setDiceValue(1);
      // Reset turn-boundary refs — new turn starting
      autoPassFiredRef.current = false;
  ...
  }, [multiplayer?.gameState?.updatedAt, multiplayer?.gameState?.hasRolled, multiplayer?.gameState?.diceValue, multiplayer?.gameState?.activePlayerColor, multiplayer?.gameState?.status, gameMode, myMultiplayerColor, players.length, isAnimating]);
  ...
  // For online mode, also show highlights when status is still transitioning
  const canShowHighlights = status === 'playing' || (gameMode === 'online' && multiplayerGameId !== null);
  const highlightedTokenIds = (canShowHighlights && hasRolled && !isAnimating && activePlayer && activePlayer.type === 'human')
  ...
  const passTurn = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (p.type !== 'none' && finishedCount < 4) {
        ...
        setHasRolled(false);
        setDiceState('idle');
        // Reset both refs for next turn
        autoPassFiredRef.current = false;
        passTurnFiredRef.current = false; // reset here for offline — online also resets via sync effect

        // Online: write turn change to Firestore
        if (gameMode === 'online' && multiplayer && multiplayerGameId) {
          const nextColor = p.color as PlayerColor;
          multiplayer.makeMove(
            ...
            0,
            false,
          ).catch(e => console.warn('passTurn Firestore write failed:', e));
        }
        return;
      }
    ...
    // Online mode: don't call endMatch locally — let Firestore drive it
    if (gameMode !== 'online') endMatch();
  };

  // Award points for online game end — called from onGameEnd callback
  const endMatchForOnline = (winners: PlayerColor[]) => {
    // Temporarily set matchWinners ref so endMatch logic works
    // endMatch reads matchWinners state which may not be set yet
    // So we call the stats/points logic directly
    const hasHuman = players.some(p => p.type === 'human' || p.type === 'online');
    const myPlayer = players.find(p => p.type === 'human');
    if (!myPlayer || !hasHuman) return;

    const isWinner = winners[0] === myPlayer.color;
    ...
    try {
      ...
      if (isWinner) {
        rewardEngine.awardPoints(rulesObj.matchWin, 'Online Match Win 🏆');
      } else {
        rewardEngine.awardPoints(rulesObj.matchLoss, 'Online Match 🤝');
      }
    } catch (err) {
      console.warn('Error awarding online points:', err);
    }
    ...
    const updatedStats: PlayerStats = {
      ...
      totalTokensFinished: stats.totalTokensFinished + tokensRef.current.filter(t => t.playerColor === myPlayer.color && t.position === 56).length,
    ...
    const newHistoryEntry: GameHistoryEntry = {
      ...
      players: players.filter(p => p.type !== 'none').map(p => ({
        name: p.name, color: p.color, type: p.type,
        rank: winners.indexOf(p.color) >= 0 ? winners.indexOf(p.color) + 1 : undefined,
      })),
      rules,
    ...
    if (fbUser && !fbUser.isAnonymous) {
      ...
      Promise.all([
        ...
        updateLeaderboard(fbUser.uid, fbUser.displayName || 'Player', fbUser.photoURL, updatedStats, wallet.availablePoints),
      ]).catch(e => console.warn('Firestore sync failed:', e));
    }
    prestigeEngine.evaluateAchievementsRealtime(true, isWinner, updatedStats, updatedHistory);
  };
  ...
  const finalizeTokenMove = (tokenId: string, rolledValue?: number) => {
    ...
    if (finalPos === 56) {
      ...
      if (rules.rollAgainOnHome) {
        ...
        setDiceState('idle');
        setIsAnimating(false);
        // Online: same player rolls again — write updated tokens + keep activePlayerColor
        if (gameMode === 'online' && multiplayer && multiplayerGameId) {
          const tokensForFirestore = updatedTokens.filter((t: any) => t.position !== 'knocked');
          multiplayer.makeMove(tokenId, tokensForFirestore, playerColor as PlayerColor, matchWinners, 0, false)
            .catch(e => console.warn('rollAgainOnHome Firestore write failed:', e));
        }
        return;
      }
    }
    ...
    if (extraTurnFromCapture && rules.rollAgainOnCapture) {
      ...
      if (gameMode === 'online' && multiplayer && multiplayerGameId) {
        const tokensForFirestore = updatedTokens.filter((t: any) => t.position !== 'knocked');
        multiplayer.makeMove(tokenId, tokensForFirestore, playerColor as PlayerColor, matchWinners, 0, false)
          .catch(e => console.warn('rollAgainOnCapture Firestore write failed:', e));
      }
      return;
    }
    ...
    if (gameMode === 'online' && multiplayer && multiplayerGameId) {
      ...
      multiplayer.makeMove(
        ...
        false, // always false — next action is always a fresh roll (extra turn or new turn)
  ...
  const handleRollDice = async () => {
    ...
    // Online mode — show rolling animation immediately, write to Firestore
    // Game logic (auto-pass, highlights) runs reactively via sync effect
    if (gameMode === 'online' && multiplayer) {
      if (!multiplayer.isMyTurn || hasRolled || diceState === 'rolling' || diceState === 'rolled') return;
      setDiceState('rolling'); // immediate visual feedback before Firestore write
      try {
        await multiplayer.rollDice();
        // sync effect sets diceState='rolled' + diceValue when snapshot arrives
      } catch (e: any) {
        console.error('Dice roll failed:', e?.code, e?.message);
        setDiceState('idle'); // reset on failure
      }
      return;
    }
  ...
  const handleTokenSelect = (tokenId: string) => {
    ...
    // Use activeColor match (isMyTurn is false when hasRolled=true by design)
    if (gameMode === 'online') {
      const isMyActiveColor = multiplayer?.gameState?.activePlayerColor === myMultiplayerColor;
      if (!isMyActiveColor) {
            return;
      }
    }
    // FIX: use synchronously-updated gameStateRef value — never stale
    // getLatestDiceValue() reads from gameStateRef.current which is updated
    // directly in onSnapshot before any React re-render
    const effectiveDice = (gameMode === 'online' && multiplayer?.getLatestDiceValue)
      ? (multiplayer.getLatestDiceValue() || diceValue)
      : diceValue;

    // DIAGNOSTIC: log all dice values on every token tap in online mode
    if (gameMode === 'online') {
      const refDice = multiplayer?.getLatestDiceValue?.() ?? 'N/A';
      const reactStateDice = multiplayer?.gameState?.diceValue ?? 'N/A';
      const localDice = diceValue;
      const validMovesCheck = getValidTokenMoves(activePlayer.color, effectiveDice);
      console.log('[TOKEN-TAP]', {
        timestamp: Date.now(),
        tokenId,
        refDice,          // from gameStateRef.current (synchronous)
        reactStateDice,   // from React state (may lag)
        localDice,        // local React diceValue state
        effectiveDice,    // what we actually use
        validMoves: validMovesCheck.length,
        rejected: !validMovesCheck.includes(tokenId),
      });
    }
    const validMoves = getValidTokenMoves(activePlayer.color, effectiveDice);
    ...
    if (validMoves.includes(tokenId)) {
      ...
      if (token) {
        ...
        const steps = token.position === 'yard' ? 1 : effectiveDice;
        moveTokenStepByStep(tokenId, steps, effectiveDice);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    // Online mode — handle auto-pass when no valid moves after roll
    if (gameMode === 'online') {
      const gs = multiplayer?.gameState;
      const isMyColor = gs?.activePlayerColor === myMultiplayerColor;
      // Guard: only auto-pass when BOTH local state AND Firestore agree hasRolled=true
      // AND it's my color in Firestore — prevents stale state from previous player's roll
      if (gs?.hasRolled && (gs?.diceValue ?? 0) > 0 && isMyColor && !autoPassFiredRef.current) {
        const validMoves = getValidTokenMoves(activePlayer.color, gs.diceValue);
        if (validMoves.length === 0) {
        ...
        if (validMoves.length === 1) {
          // Single valid move — auto move token
          autoPassFiredRef.current = true;
          const onlyToken = tokens.find(t => t.id === validMoves[0]);
          if (onlyToken) {
  ...
  }, [status, activePlayer, hasRolled, diceValue, diceState, isPaused, isAnimating, gameMode, multiplayer?.isMyTurn]);
            ...
            <motion.div key="room" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}}
              ...
              <TableRoom
                ...
                onGameReady={(gameId, gameData, color, mode, tNum) => {
                  ...
                  setActiveTableId(activeTableId || gameData.tableId || ''); // ensure tableId is set
