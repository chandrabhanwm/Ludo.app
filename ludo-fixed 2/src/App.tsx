...
export default function App() {
  ...
  const multiplayer = useMultiplayerGame(
    multiplayerGameId && activeTableId && myMultiplayerColor ? {
  ...
  // Tracks the last dice roll (value + hasRolled) we've already animated locally,
  // so the online sync effect only plays the roll animation once per actual roll.
  const onlineDiceSyncRef = useRef<{ diceValue: number; hasRolled: boolean } | null>(null);
  const onlineDiceTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const onlineAutoActionTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const passTurn = () => {
    ...
    // Filter players that are actually in the game and haven't fully finished (won 4 tokens)
    let nextIdx = (currentPlayerIndex + 1) % players.length;

    // Loop up to 4 times to find next active player who hasn't finished
    for (let i = 0; i < 4; i++) {
      const p = players[nextIdx];
      const finishedCount = tokens.filter(t => t.playerColor === p.color && t.position === 56).length;
    ...
    // If we can't find any eligible player, end match
    endMatch();
  };
  ...
  const finalizeTokenMove = (tokenId: string, rolledValue?: number) => {
    ...
    // Live snapshot of winners for this function call — avoids races with async setState
    // when this move also needs to be broadcast to Firestore for online games.
    let winnersSnapshot = matchWinners;

    // Helper: figure out who plays next (used for the online Firestore write, mirrors passTurn())
    const computeNextOnlineColor = (tokensSnapshot: Token[]): PlayerColor | null => {
      let nextIdx = (currentPlayerIndex + 1) % players.length;
      for (let i = 0; i < players.length; i++) {
        const p = players[nextIdx];
        const finishedCount = tokensSnapshot.filter(t => t.playerColor === p.color && t.position === 56).length;
        if (p.type !== 'none' && finishedCount < 4) {
          return p.color;
        }
        nextIdx = (nextIdx + 1) % players.length;
    ...
    if (finalPos === 56) {
      ...
      if (finishedCount === 4) {
        ...
        winnersSnapshot = [...matchWinners, playerColor];
        setMatchWinners(winnersSnapshot);
        ...
        if (remainingUnfinished.length <= 1 || ranking >= activeCount - 1) {
          if (gameMode === 'online' && multiplayer) {
            multiplayer.makeMove(tokenId, updatedTokens, playerColor, winnersSnapshot, 0, false);
          }
          endMatch();
          setIsAnimating(false);
          return;
    ...
    if (effectiveDiceValue === 6 && rules.rollAgainOnSix) {
      // consecutiveSixes >= 2 is already handled at roll time — just increment and give extra turn
      addLog(`🎁 Extra Roll for rolling a 6!`, playerColor);
      if (gameMode === 'online' && multiplayer) {
        multiplayer.makeMove(tokenId, updatedTokens, playerColor, winnersSnapshot, consecutiveSixes + 1, false);
        setIsAnimating(false);
        return;
      }
      ...
      if (gameMode === 'online' && multiplayer) {
        const nextColor = computeNextOnlineColor(updatedTokens);
        if (nextColor) {
          multiplayer.makeMove(tokenId, updatedTokens, nextColor, winnersSnapshot, 0, false);
        }
        setIsAnimating(false);
        return;
      }
  ...
  const handleRollDice = async () => {
    // Online mode — write to Firestore instead
    if (gameMode === 'online' && multiplayer) {
      if (!multiplayer.isMyTurn || isAnimating || diceState === 'rolling' || hasRolled) return;
  ...
  // Firestore (via useMultiplayerGame) is the source of truth for online
  // games. This effect pulls that shared state down into the same local
  // state (players/tokens/currentPlayerIndex/dice) that the Board & dice
  // UI already read, for every client — including the one who just acted.
  // Without this, the local UI never advances (dice stays disabled, moves
  // never appear) because nothing ever consumed `multiplayer.gameState`.
  useEffect(() => {
    if (gameMode !== 'online' || !multiplayer || !multiplayer.gameState) return;
    if (status !== 'playing') return;
    const gs = multiplayer.gameState;

    // 1. Sync connection/knockout/finish info onto existing player slots
    //    (keep locally-known name/avatar/type, only merge live fields)
    setPlayers(prev => prev.map(p => {
      if (p.type === 'none') return p;
      const mp = gs.players.find(x => x.color === p.color);
      if (!mp) return p;
      const finishedPosition = mp.finishedPosition ?? undefined;
      return p.finishedPosition === finishedPosition ? p : { ...p, finishedPosition };
    }));

    // 2. Sync board tokens — skip while a local hop animation is mid-flight
    //    so we don't cut off our own smooth move animation.
    if (!isAnimating) {
      setTokens(prev => {
        const same = prev.length === gs.tokens.length &&
          prev.every((t, i) => gs.tokens[i] && t.id === gs.tokens[i].id && t.position === gs.tokens[i].position);
        return same ? prev : gs.tokens;
      });
    }

    // 3. Sync whose turn it is
    const activeIdx = players.findIndex(p => p.color === gs.activePlayerColor);
    if (activeIdx >= 0 && activeIdx !== currentPlayerIndex) {
      setCurrentPlayerIndex(activeIdx);
    }

    // 4. Sync match winners
    setMatchWinners(prev => {
      const same = prev.length === gs.matchWinners.length && prev.every((c, i) => c === gs.matchWinners[i]);
      return same ? prev : gs.matchWinners;
    });

    // 5. Sync dice — play the roll animation once per new roll, for everyone
    const prevDice = onlineDiceSyncRef.current;
    const isNewRoll = gs.hasRolled && (!prevDice || !prevDice.hasRolled || prevDice.diceValue !== gs.diceValue);
    onlineDiceSyncRef.current = { diceValue: gs.diceValue, hasRolled: gs.hasRolled };

    if (onlineDiceTimeoutRef.current) {
      clearTimeout(onlineDiceTimeoutRef.current);
      onlineDiceTimeoutRef.current = null;
    }
    ...
    if (!gs.hasRolled) {
      ...
      setDiceState('rolling');
      setIsAnimating(true);
      onlineDiceTimeoutRef.current = setTimeout(() => {
        setDiceValue(gs.diceValue);
        setDiceState('rolled');
        setHasRolled(true);
        setIsAnimating(false);
        addLog(`Rolled a ${gs.diceValue}`, gs.activePlayerColor);

        // If it's my turn, auto-handle "no valid moves" / "only one token can move"
        // — mirrors the local single-player behaviour in handleRollDice().
        if (myMultiplayerColor && gs.activePlayerColor === myMultiplayerColor) {
          const validMoves = getValidTokenMoves(myMultiplayerColor, gs.diceValue);
          if (onlineAutoActionTimeoutRef.current) clearTimeout(onlineAutoActionTimeoutRef.current);

          if (validMoves.length === 0) {
            addLog(`No valid moves! Turn passing...`, myMultiplayerColor);
            onlineAutoActionTimeoutRef.current = setTimeout(() => {
              const myIdx = players.findIndex(p => p.color === myMultiplayerColor);
              let nextIdx = (myIdx + 1) % players.length;
              let nextColor: PlayerColor | null = null;
              for (let i = 0; i < players.length; i++) {
                const p = players[nextIdx];
                const finishedCount = gs.tokens.filter(t => t.playerColor === p.color && t.position === 56).length;
                if (p.type !== 'none' && finishedCount < 4) { nextColor = p.color; break; }
                nextIdx = (nextIdx + 1) % players.length;
              }
              if (nextColor) {
                multiplayer.makeMove('', gs.tokens, nextColor, gs.matchWinners, 0, false);
              }
            }, 1300);
          } else if (validMoves.length === 1) {
            const onlyToken = gs.tokens.find(t => t.id === validMoves[0]);
            if (onlyToken) {
              onlineAutoActionTimeoutRef.current = setTimeout(() => {
                const steps = onlyToken.position === 'yard' ? 1 : gs.diceValue;
                moveTokenStepByStep(validMoves[0], steps, gs.diceValue);
              }, 600);
            }
          }
        }
    ...
    return () => {
      if (onlineDiceTimeoutRef.current) clearTimeout(onlineDiceTimeoutRef.current);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gameMode, multiplayer?.gameState, status]);
      ...
      {gameMode === 'online' && multiplayer?.disconnectingPlayer && (
        <div className="fixed top-16 left-0 right-0 z-50 flex justify-center px-4">
          <div className="flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold"
            style={{background:'rgba(245,158,11,0.95)',color:'#78350f',boxShadow:'0 4px 16px rgba(0,0,0,0.2)'}}>
            <span className="animate-spin">⏳</span>
            <span>{multiplayer.disconnectingPlayer.displayName} reconnecting... {multiplayer.disconnectSecondsLeft}s</span>
          </div>
        </div>
      )}
        ...
        <AnimatePresence mode="wait">
          ...
          {status === 'menu' && onlineScreen === 'room' && activeTableId && (
            <motion.div key="room" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}}
              ...
              <TableRoom
                ...
                onGameStart={async (gameId, color, mode) => {
                  setMultiplayerGameId(gameId);
                  setMyMultiplayerColor(color as PlayerColor);
                  setActiveTableMode(mode);
                  setOnlineScreen(null);

                  // Wait briefly for game doc to be created then initialize board
                  setTimeout(async () => {
                    try {
                      ...
                      const gameDoc = snap.data();

                      // Convert Firestore players → game engine Player[]
                      const allColors: PlayerColor[] = ['red','green','blue','yellow'];
                      const gamePlayers: Player[] = allColors.map(c => {
                        const mp = gameDoc.players?.find((p: any) => p.color === c);
                        if (!mp) return { id: c, name: '', color: c, type: 'none' as PlayerType, avatar: '⚪', isWinner: false };
                        const isMe = mp.uid === firebaseUserRef.current?.uid;
                        return {
                          id: c,
                          name: mp.displayName,
                          color: c,
                          type: isMe ? 'human' : 'online' as PlayerType,
                          avatar: mp.photoURL || '👤',
                          isWinner: false,
                        };
                      });
                      ...
                      if (gameDoc.tokens) {
                        setTokens(gameDoc.tokens);
                      } else {
                        const newTokens: Token[] = [];
                        gamePlayers.forEach(p => {
                          if (p.type !== 'none') {
                            for (let i = 0; i < 4; i++) {
                              newTokens.push({ id: `${p.color}-${i}`, playerColor: p.color, idInColor: i, position: 'yard' });
                            }
                          }
                        });
                        setTokens(newTokens);
                      ...
                      setConsecutiveSixes(0);
                      setGameLogs([]);
                      const startingColor = gameDoc.activePlayerColor;
                      const startIdx = gamePlayers.findIndex(p => p.color === startingColor && p.type !== 'none');
                      const firstActive = gamePlayers.findIndex(p => p.type !== 'none');
                      setCurrentPlayerIndex(startIdx >= 0 ? startIdx : (firstActive >= 0 ? firstActive : 0));
                      matchStartTime.current = Date.now();
                      setStatus('playing');
                    } catch (e) {
                      console.error('Failed to initialize multiplayer game:', e);
                    }
                  }, 1500);
                }}
          ...
          {status === 'gameover' && (
            <EndGameScreen
              players={players}
              matchWinners={matchWinners}
              onRematch={() => startNewMatch(players, rules)}
