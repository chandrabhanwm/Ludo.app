...
export const Board: React.FC<BoardProps> = ({
  ...
  const getStackOffset = (token: Token) => {
    ...
    // Diagonal staircase fan — each token peeks from behind, no scale reduction
    // Front token (idx=0) is top-left, each subsequent token offset bottom-right
    const step = 22; // px offset per token
    const offsetPx = (count - 1 - idx) * step;
