...
export const Board: React.FC<BoardProps> = ({
  ...
  const getStackOffset = (token: Token) => {
    ...
    // Diagonal staircase — pixel offset relative to cell size so it's visible on mobile
    // Cell width = boardSize/15 but we don't have boardSize here, so use % of token
    // 30% of token width at 88% cell = visibly peeks out
    const stepPct = 28; // % of token's own size per step
    const offsetPct = (count - 1 - idx) * stepPct;
  ...
  const renderToken = (token: Token, inYard = false) => {
    ...
    const activeColors = {
      red: {
        innerBg: 'from-[#fda4af] via-[#f43f5e] to-[#881337]',
        glow: 'rgba(244,63,94,0.4)',
        dotBg: 'bg-[#be123c]',
        borderColor: '#be123c',
      },
      green: {
        innerBg: 'from-[#bbf7d0] via-[#22c55e] to-[#14532d]',
        glow: 'rgba(34,197,94,0.4)',
        dotBg: 'bg-[#15803d]',
        borderColor: '#16a34a',
      },
      yellow: {
        innerBg: 'from-[#fef9c3] via-[#f59e0b] to-[#78350f]',
        glow: 'rgba(245,158,11,0.4)',
        dotBg: 'bg-[#b45309]',
        borderColor: '#d97706',
      },
      blue: {
        innerBg: 'from-[#bfdbfe] via-[#3b82f6] to-[#1e3a8a]',
      ...
      >
        ...
        <motion.div
          style={{
            ...
            y: inYard ? 0 : oy,
            borderRadius: '50%',
            border: `1.5px solid ${activeColors[token.playerColor].borderColor}`,
          }}
          animate={
          ...
          className={`pointer-events-auto relative ${sizeClasses} rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 overflow-hidden ${
            isGameOver
              ? isWinnerColor
              ...
              : isClickable
              ? `ring-[3.5px] ${t.ring} ring-offset-2 shadow-[0_0_22px_${activeColors[token.playerColor].glow},0_8px_18px_rgba(0,0,0,0.26)]`
              : `shadow-[0_6px_14px_rgba(0,0,0,0.2),0_0_10px_${activeColors[token.playerColor].glow}] ${
                  !isClickable && !isBeingCaptured && !experienceEngine.getConfig().reducedMotion ? 'living-token-idle' : ''
                }`
          }`}
        >
          {/* Full-bleed color gradient — no outer shell */}
          <div className={`absolute inset-0 rounded-full bg-gradient-to-b ${activeColors[token.playerColor].innerBg}`} />

          {/* Top gloss dome */}
          <div className="absolute top-0 left-0 right-0 h-[46%] bg-gradient-to-b from-white/55 via-white/10 to-transparent rounded-full pointer-events-none z-10" />
          {/* Gloss spot */}
          <div className="absolute top-[10%] left-[14%] w-[30%] h-[22%] bg-white/50 rounded-full blur-[1px] pointer-events-none z-10" />

          {/* White eye-lens + color core */}
          <div className="w-[40%] h-[40%] rounded-full bg-white flex items-center justify-center shadow-[0_1.5px_3px_rgba(0,0,0,0.2)] relative z-20">
          ...
          {/* Stack count badge — shown on front token only */}
          {!inYard && token.position !== 56 && (() => {
            if (token.position === 'yard') return null;
            const coord = getTokenCoordinates(token);
            const key = `${coord.row.toFixed(1)}-${coord.col.toFixed(1)}`;
            const group = coordinateGroups[key] || [];
            const isFrontToken = group.length > 1 && group[0]?.id === token.id;
            if (!isFrontToken) return null;
            const isSameColor = group.every(t => t.playerColor === group[0].playerColor);
            return (
              <div className={`absolute -bottom-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 rounded-full border border-white flex items-center justify-center z-30 ${
                isSameColor ? 'bg-amber-500' : 'bg-stone-800'
              }`}>
                <span className="text-[7px] sm:text-[9px] text-white font-black select-none leading-none">
                  {isSameColor ? '🔒' : `×${group.length}`}
                </span>
              </div>
            );
          })()}
        ...
        <div className="flex items-center gap-1 sm:gap-2 min-w-0 flex-1">
          {/^https?:\/\//.test(player.avatar) ? (
            <img
              src={player.avatar}
              alt=""
              className="w-3.5 h-3.5 sm:w-5 sm:h-5 rounded-full object-cover shrink-0"
  ...
  const renderYard = (color: PlayerColor) => {
    ...
    // Glow only during roll phase — disappears the moment dice is rolled
    const showYardGlow = isActiveTurn && !hasRolled && diceState === 'idle';

    return (
      <div className={`absolute ${positionClasses} w-[40%] h-[40%] z-10 select-none ${
        color === "red" ? "rounded-tl-xl sm:rounded-tl-[24px]" :
        color === "green" ? "rounded-tr-xl sm:rounded-tr-[24px]" :
        color === "yellow" ? "rounded-br-xl sm:rounded-br-[24px]" :
        "rounded-bl-xl sm:rounded-bl-[24px]"
      } transition-all duration-500 ${
        isGameOver && !isWinnerYard ? 'opacity-30 filter grayscale-[25%] pointer-events-none' : ''
      } ${
        showYardGlow ? 'z-30' : ''
      }`}>
        <div className={`${getYardWrapperClasses(color)} w-full h-full relative overflow-hidden ${
          color === "red" ? "rounded-tl-xl sm:rounded-tl-[24px]" :
          color === "green" ? "rounded-tr-xl sm:rounded-tr-[24px]" :
          color === "yellow" ? "rounded-br-xl sm:rounded-br-[24px]" :
          "rounded-bl-xl sm:rounded-bl-[24px]"
        } shadow-[inset_0_4px_10px_rgba(0,0,0,0.12),0_2px_4px_rgba(255,255,255,0.06)]`}>
          ...
          {/* Active Turn — inset glow contained within yard */}
          {showYardGlow && (
            <div
              className="absolute inset-0 pointer-events-none z-20"
              style={{
                boxShadow: color === 'red'
                  ? 'inset 0 0 0 3px #f43f5e, inset 0 0 18px rgba(244,63,94,0.35)'
                  : color === 'green'
                  ? 'inset 0 0 0 3px #22c55e, inset 0 0 18px rgba(34,197,94,0.35)'
                  : color === 'yellow'
                  ? 'inset 0 0 0 3px #f59e0b, inset 0 0 18px rgba(245,158,11,0.35)'
                  : 'inset 0 0 0 3px #3b82f6, inset 0 0 18px rgba(59,130,246,0.35)'
              }}
            />
          )}
  ...
  return (
    <div className="relative w-full max-w-full sm:max-w-[700px] md:max-w-[780px] lg:max-w-[900px] xl:max-w-[1020px] 2xl:max-w-[1120px] mx-auto px-0 select-none">
      ...
      } ${activeTheme.isPhysical ? 'bg-[#faf3e3]' : 'bg-[#faf8f5]'} rounded-none overflow-hidden flex items-center justify-center`}>
        <svg
          viewBox="0 0 100 100"
