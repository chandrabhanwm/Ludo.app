...
import { listenToTables, initializeTables } from '../multiplayer/gameSync';
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Tables may already exist or rules block write — still try to listen
        console.warn('initializeTables:', e);
      }
      // Start listening regardless
      unsub = listenToTables((data) => {
        setTables(data);
        setLoading(false);
      });
    };
  ...
  const renderSlotButton = (tableId: string, cfg: typeof TABLES[0], mode: TableMode) => {
    ...
    return (
      <motion.button
        ...
        onClick={() => canJoin && onJoinTable(tableId, mode)}
        disabled={!canJoin}
