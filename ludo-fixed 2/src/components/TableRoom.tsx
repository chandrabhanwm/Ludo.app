...
interface TableRoomProps {
  ...
  onGameStart: (gameId: string, color: string, mode: TableMode) => void;
  onBack: () => void;
}
...
export const TableRoom: React.FC<TableRoomProps> = ({
  tableId, uid, displayName, photoURL, onGameStart, onBack,
}) => {
  ...
  // Get current slot
  const slot = tableData ? (mode === '2P' ? tableData.slots2P : tableData.slots4P) : null;
    ...
    const tick = setInterval(() => {
      ...
      if (remaining === 0) {
        ...
        if (isInSlot && myPlayer && slot?.players?.[0]?.uid === uid) {
          const gameId = `${tableId}_${mode}_${slot.countdownStartedAt}`;
          ...
          await import('../multiplayer/gameSync').then(({ createGame, saveUserCurrentGame }) => {
            createGame(gameId, tableId, mode, mpPlayers, initialTokens);
            slot.players.forEach(p => saveUserCurrentGame(p.uid, gameId, tableId, p.color));
          });
        }
        if (isInSlot && myPlayer) {
          const gameId = `${tableId}_${mode}_${slot.countdownStartedAt}`;
          onGameStart(gameId, myPlayer.color, mode);
        }
        };
        doStart();
      }
  ...
  const handleModeSwitch = async (newMode: TableMode) => {
    ...
    if (isInSlot && slot?.status === 'waiting') {
      await leaveTable(tableId, mode, uid);
      setMyColor(null);
    }
    setMode(newMode);
    setError(null);
  ...
  const handleJoin = async () => {
    ...
    setJoining(true);
    setError(null);
    const result = await joinTable(tableId, mode, uid, displayName, photoURL);
