    ...
    const tick = setInterval(() => {
      ...
      if (remaining === 0) {
        ...
        if (isInSlot && myPlayer && slot?.players?.[0]?.uid === uid) {
          ...
          await import('../multiplayer/gameSync').then(async ({ createGame, saveUserCurrentGame }) => {
            await createGame(gameId, tableId, tableNum, mode, mpPlayers, initialTokens);
            await Promise.all(slot.players.map(p => saveUserCurrentGame(p.uid, gameId, tableId, tableNum, p.color)));
          });
        }
