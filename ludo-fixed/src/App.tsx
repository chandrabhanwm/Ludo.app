...
import { Player, Token, GameRules, GameLog, GameHistoryEntry, PlayerStats, PlayerColor, DiceState, GameStatus } from './types';
...
export default function App() {
  ...
  // Auth state — check localStorage for returning user
  const [isLoggedIn, setIsLoggedIn] = useState<boolean>(() => {
    try { return localStorage.getItem('ludo_is_logged_in') === 'true'; } catch { return false; }
  });
  const [isGuest, setIsGuest] = useState<boolean>(() => {
    try { return localStorage.getItem('ludo_is_guest') === 'true'; } catch { return false; }
  });
  const [lockPopup, setLockPopup] = useState<'Tournament' | 'Rewards' | null>(null);

  const [status, setStatus] = useState<GameStatus>(() => {
    // If returning user (logged in or guest) go straight to menu
    try {
      const loggedIn = localStorage.getItem('ludo_is_logged_in') === 'true';
      const guest = localStorage.getItem('ludo_is_guest') === 'true';
      if (loggedIn || guest) return 'menu';
    } catch {}
    return 'login';
  });
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      // Cap size strictly by both width (vw - 16) and height (vh * 0.815 - 16) to ensure a perfect 1:1 square
      const maxAvailableHeight = Math.floor(vh * 0.815) - 16;
  ...
  const moveTokenStepByStep = (tokenId: string, stepsToMove: number) => {
  ...
  const finalizeTokenMove = (tokenId: string) => {
    ...
    if (diceValue === 6 && rules.rollAgainOnSix) {
      // Limit to 3 consecutive sixes
      if (consecutiveSixes >= 2) {
        addLog(`⚠️ 3 consecutive sixes rolled! Turn passed.`, playerColor);
        setConsecutiveSixes(0);
        passTurn();
      } else {
  ...
  const handleTokenSelect = (tokenId: string) => {
    ...
    if (validMoves.includes(tokenId)) {
      ...
      if (token) {
        ...
        moveTokenStepByStep(tokenId, steps);
      }
    }
  };
      ...
      if (!hasRolled && diceState === 'idle') {
        ...
        if (validMoves.length > 0) {
          aiTimeoutRef.current = setTimeout(() => {
            ...
            moveTokenStepByStep(decision.tokenId, stepsToTake);
        ...
        <AnimatePresence mode="wait">
          {status === 'login' && (
            ...
            >
              <LoginScreen
                onGoogleLogin={() => {
                  // Simulate Google login — replace with real Firebase Auth later
                  localStorage.setItem('ludo_is_logged_in', 'true');
                  localStorage.setItem('ludo_is_guest', 'false');
                  localStorage.setItem('ludo_player_name', 'Player');
                  setIsLoggedIn(true);
                  setIsGuest(false);
                  setStatus('menu');
                }}
          ...
          {(status === 'playing' || status === 'gameover') && (
            ...
            >
              ...
              <div className="lg:hidden sticky top-0 left-0 right-0 w-full backdrop-blur-md border-b z-30 px-3 flex items-center justify-between h-[10dvh] min-h-[44px] max-h-[48px] shrink-0 select-none" style={{background:'rgba(253,248,240,0.92)',borderColor:'rgba(180,120,60,0.18)'}}>
              ...
              <div className="w-full h-[90dvh] lg:h-auto flex flex-col lg:grid lg:grid-cols-12 gap-0 lg:gap-5 items-center lg:items-start px-0 lg:px-2 py-0">
                {/* Left sidebar UI panel */}
                <div className={`lg:col-span-4 flex-col gap-2 sm:gap-4 order-1 lg:order-1 ${mobileActiveTab === 'leaderboard' ? 'flex h-[90dvh] overflow-y-auto px-2 py-2' : 'hidden lg:flex'}`}>
                  <GameHUD
                    players={players}
                    tokens={tokens}
                ...
                <div className={`lg:col-span-8 flex flex-col items-center justify-between order-2 lg:order-2 w-full ${mobileActiveTab === 'board' ? 'flex h-[90dvh]' : 'hidden lg:flex'} px-0 py-0 overflow-hidden`}>
                  
                  {/* Ludo Board Area: Occupies exactly 81.5% of screen height on mobile, centering the square board */}
                  <div className="w-full h-[81.5dvh] lg:h-auto flex items-center justify-center overflow-hidden shrink-0">
                    <div
                      style={boardSize > 0 ? { width: boardSize, height: boardSize } : undefined}
                      className="mx-auto shrink-0 transition-all duration-300"
                    >
                      {!isFullScreenMode && (
                        <Board
                          tokens={tokens}
                        ...
                        <div className="flex items-center gap-2 xs:gap-3 sm:gap-4 z-10 min-w-0">
                          ...
                          <div className="flex flex-col min-w-0">
                            <span className="text-[7.5px] sm:text-[10px] font-black uppercase tracking-widest text-amber-700 leading-none">
                              {activePlayer?.type === 'computer' ? 'AI Turn' : 'Active Turn'}
                            </span>
